const listaDePersonagens = ["viajante hydro", "viajante geo", "viajante anemo", "viajante electro", "viajante dendro",
...
function randomizarViajante(time){
    for(let i = 0; i < 3; i++){
        if(time[i]===4){
            time[i] = Math.round(Math.random() * (4 - 0));
        }
    }
}

function randomizarPersonagens(){
    let personagem = Math.round(Math.random() * (78 - 0));
    if(personagem < 4){
        personagem = 4
    }
    return personagem;
...
randomizarViajante(time1)
const time2 = times.slice(4, 8);
randomizarViajante(time2)
